function [net, info] = dldl_train_ml(net, imdb, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(imdb.images.set<=2) ; end
  ...
  switch opts.errorFunction
    case 'none'
      ...
      case 'multiclass'
          ...
          if isempty(opts.errorLabels), opts.errorLabels = {'map', 'map11'} ; end
...
for epoch=start+1:opts.numEpochs
  ...
  for f = sets
    ...
    info.(f).error(:,epoch) = stats.(f)(3:end) ;
  end
  if ~evaluateMode
     tic ;
    if mod(epoch, 10) ==0
...
% -------------------------------------------------------------------------
function predictions = error_regression(opts, labels, res)
% -------------------------------------------------------------------------
    ...
    case 'ml' 
        switch opts.loss
            case {'sigmoid_cross_entropy_loss','ls_sigmoid_cross_entropy_loss'}
                 predictions = vl_nnsigmoid(predictions);
            case 'cross_entropy_loss'
        end
        
        
%         for ci =1 :20
%             [~,~,info] = vl_pr(labels(ci,:),mapminmax(double(predictions(ci,:)))) ;
%             ap(ci) = info.ap ;
%             ap11(ci) = info.ap_interp_11 ;
%         end
end
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    % evaluate the CNN
    net.layers{end}.class = labels.label_ld ;
    if training, dzdy = one; else, dzdy = [] ; end
    res = dldl_simplenn(net, im, dzdy, res, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
...
%       reshape(opts.errorFunction(opts, labels.label, res),[],1) ; ]],2) ;
  ...
  pred_label = [pred_label opts.errorFunction(opts, labels.label, res)];
  real_label = [real_label labels.label];
 
  for ci =1 :20
      [~,~,info] = vl_pr(real_label(ci,:),double(pred_label(ci,:))) ;
      ap(ci) = info.ap ;
      ap11(ci) = info.ap_interp_11 ;
      if isnan(ap(ci))
          ap(ci) = 0;
  ...
  time = toc(start) ;
  stats(3:4) = [ap(ci+1),ap11(ci+1)].*100;
%   stats = sum([stats,[0 ; error]],2); % works even when stats=[]
  
  stats(1) = time ;
  stats(2) = stats(2) + double(gather(res(end).x));
  ...
  for i=1:numel(opts.errorLabels)
    fprintf(' %s:%f', opts.errorLabels{i}, stats(2+i)) ;
  end
