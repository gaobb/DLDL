function res = true_dldl_simplenn(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'l2loss'
            res(i+1).x =vl_l2loss(res(i).x, l.class) ;   
    case 'l1loss'
            res(i+1).x =vl_l1loss(res(i).x, l.class) ;           
    case 'klloss'
            res(i+1).x =vl_klloss(res(i).x, l.class) ;
    case 'klgloss'
            res(i+1).x =vl_klgloss(res(i).x, l.class) ;        
    case 'hellingerloss'
            res(i+1).x =vl_hellingerloss(res(i).x, l.class) ;        
    case 'jdloss'
            res(i+1).x =vl_jdloss(res(i).x, l.class) ;  
    case 'tukeyloss'
            res(i+1).x =vl_tukeyloss(res(i).x, l.class) ;  
    case 'epsilonloss'
            res(i+1).x =vl_epsilonloss(res(i).x, l.class) ;              
    case 'relu'
      if isfield(l, 'leak'), leak = {'leak', l.leak} ; else leak = {} ; end
      res(i+1).x = vl_nnrelu(res(i).x,[],leak{:}) ;
    case 'prelu'
      res(i+1).x = vl_nnprelu(res(i).x,l.weights{1}) ;  
    case 'sigmoid'
  ...
  forget = forget & ~(strcmp(l.type, 'loss') || strcmp(l.type, 'softmaxloss')||strcmp(l.type, 'klloss')...
                ||strcmp(l.type, 'jdloss' ) || strcmp(l.type, 'softmax')|| strcmp(l.type, 'l2loss') ...
                || strcmp(l.type, 'l1loss') || strcmp(l.type, 'tukeyloss')|| strcmp(l.type, 'epsilonloss')...
                || strcmp(l.type, 'hellingerloss') || strcmp(l.type, 'klgloss')) ;
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'klloss'
                res(i).dzdx =vl_klloss(res(i).x, l.class,res(i+1).dzdx) ;
      case 'klgloss'
                res(i).dzdx =vl_klgloss(res(i).x, l.class,res(i+1).dzdx) ;          
      case 'hellingerloss'
                res(i).dzdx =vl_hellingerloss(res(i).x, l.class,res(i+1).dzdx) ;          
      case 'l2loss'
                res(i).dzdx =vl_l2loss(res(i).x, l.class,res(i+1).dzdx) ;    
      case 'l1loss'
                res(i).dzdx =vl_l1loss(res(i).x, l.class,res(i+1).dzdx);
      case 'tukeyloss'
                res(i).dzdx =vl_tukeyloss(res(i).x, l.class,res(i+1).dzdx);  
      case 'epsilonloss'
                res(i).dzdx =vl_epsilonloss(res(i).x, l.class,res(i+1).dzdx);                 
      case 'jdloss'
                res(i).dzdx =vl_jdloss(res(i).x, l.class,res(i+1).dzdx) ;    
      case 'relu'
        if isfield(l, 'leak'), leak = {'leak', l.leak} ; else leak = {} ; end
        if ~isempty(res(i).x)
      ...
      case 'prelu'
                 if ~opts.accumulate
                    if isfield(l, 'weights')
                        [res(i).dzdx, res(i).dzdw{1}] = ...
                            vl_nnprelu(res(i).x, l.weights{1}, ...
                            res(i+1).dzdx) ;
                    else
                        [res(i).dzdx, res(i).dzdw{1}] = ...
                            vl_nnprelu(res(i).x, l.filters, ...
                            res(i+1).dzdx) ;
                    end
                else
                    dzdw = cell(1,1) ;
