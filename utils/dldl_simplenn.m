function res = dldl_simplenn(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'hcp'
      res(i+1).x = vl_hcp(res(i).x);
   
    case 'normalizeto1'
      res(i+1).x = vl_normalizeto1(res(i).x); 
      
    case 'smklloss'
      res(i+1).x = vl_smklloss(res(i).x, l.class) ;
    ...
    case 'sigmoid_cross_entropy_loss'
      res(i+1).x = vl_sigmoid_cross_entropy_loss(res(i).x, l.class);  
      
    case 'cross_entropy_loss'
      res(i+1).x = vl_cross_entropy_loss(res(i).x, l.class);    
    ...
    case 'prelu'
      res(i+1).x = vl_prelu(res(i).x,l.weights{1}) ;  
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'hcp'
        res(i).dzdx = vl_hcp(res(i).x, res(i+1).dzdx);   
        
      case 'smklloss'
        res(i).dzdx = vl_smklloss(res(i).x, l.class, res(i+1).dzdx) ;  
      case 'klloss'
      ...
      case 'sigmoid_cross_entropy_loss'
        res(i).dzdx = vl_sigmoid_cross_entropy_loss(res(i).x, l.class, res(i+1).dzdx);  
        
      case 'cross_entropy_loss'
        res(i).dzdx = vl_cross_entropy_loss(res(i).x, l.class, res(i+1).dzdx); 
      case 'relu'
        if l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
        if ~isempty(res(i).x)
                 ...
                 if ~opts.accumulate
                    if isfield(l, 'weights')
                        [res(i).dzdx, res(i).dzdw{1}] = ...
                            vl_prelu(res(i).x, l.weights{1}, ...
                            res(i+1).dzdx) ;
                    else
                        [res(i).dzdx, res(i).dzdw{1}] = ...
                            vl_prelu(res(i).x, l.filters, ...
                            res(i+1).dzdx) ;
                    end
                else
                    dzdw = cell(1,1) ;
