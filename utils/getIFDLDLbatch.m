function imo = getIFDLDLbatch(images, varargin)
...
%   end
  ...
  switch opts.transformation
      case 'stretch'
  opts.border(1) = round(rand*256);
  opts.border(2) = opts.border(1);
      case 'none'
          opts.border(1) = 0;
          opts.border(2) = 0;
  end
...
%       end
      ...
      imo(:,:,:,si) = bsxfun(@minus, imt(sy,sx,:), imresize(offset,opts.imageSize(1:2))) ;
    else
      imo(:,:,:,si) = imt(sy,sx,:) ;
    end
    si = si + 1 ;
