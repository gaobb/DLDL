function [net, info] = dldl_age_train(net, imdb, getBatch, varargin)
...
%             if isempty(opts.errorLabels), opts.errorLabels = {'top1e', 'top5e'} ; end
             if isempty(opts.errorLabels), opts.errorLabels = {'Mae','ChaLearnError'} ; end
%             if isempty(opts.errorLabels),opts.errorLabels = {'sc',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '};end
...
%   sets = {'val'} ; 
  ...
  fid  = fopen(['chalearn_',opts.loss,'.txt'],'a');
  fprintf(fid,'%f\n',info.(f).error);
  fclose(fid);
  figure(1) ; clf ;
...
pre_pos = squeeze(predictions);
pre_age = pre_pos(1,:);
rea_age = labels.label;

err(1,1) = sum(abs(pre_age-rea_age));
err(2,1) = sum(1-exp(-(pre_age-rea_age).^2./( 2.*(labels.delta.^2))));
     ...
     case 'age'
         switch opts.loss
             case {'l2loss','l1loss','epsilonloss','tukeyloss'}
                 [~,ps] = mapminmax([1:85],-1,1);
                 pred_age = mapminmax('reverse',predictions',ps);
                 real_age = mapminmax('reverse',labels.label,ps);
             otherwise
                 label_set = opts.label_set;
                 [~,p_ind] = max(predictions);
                 pred_age = label_set(p_ind);
                 %pre_age = [1:85]*predictions;
                 [~,r_ind] = max(labels.label);
                 real_age = label_set(r_ind);
         end
         
         err(1,1) = sum(abs(pred_age-real_age)); 
         err(2,1) = sum(1-exp(-(pred_age-real_age).^2./(2.*(labels.delta.^2))));
 end
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    % evaluate CNN
    net.layers{end}.class = labels.label ;
