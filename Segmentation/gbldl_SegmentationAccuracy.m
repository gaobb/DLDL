classdef gbldl_SegmentationAccuracy < dagnn.Loss
...
%       labels = int32(labels(:, :, 22)) + 1;
%       labels(labels == 255) = 0;
%       labels = single(labels(ly,lx)) ;
      labels = mod(single(labels(:,:,22)) + 1, 256) ; % 0 = ignore, 1 = bkg
    ...
    function obj = gbldl_SegmentationAccuracy(varargin)
      obj.load(varargin) ;
    end
  end
end